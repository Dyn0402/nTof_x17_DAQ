...
def main():
    ...
    if len(sys.argv) < 3:
        print('Usage: dream_daq_control_hold.py <yaml_config_file_path> <sub_run_name>')
        sys.exit(1)
    config_file_path = sys.argv[1]
    sub_run_name = sys.argv[2]  # 'start' or 'stop'
    # cfg = load_yaml_config(config_file_path)
    cfg = RunConfigBase(config_file_path)

    # Check that dream_daq_info exists in config
    if not hasattr(cfg, 'dream_daq_info'):
        print('Error: dream_daq_info not found in config file.')
        sys.exit(1)
    ...
    run_command = ['/home/dylan/bin/RunCtrl', '-c', cfg_run_path, '-f', sub_run_name, '-b']
