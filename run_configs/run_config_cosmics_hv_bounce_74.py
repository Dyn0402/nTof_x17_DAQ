...
# -*- coding: utf-8 -*-
"""
run_config_cosmics_hv_bounce_74.py — run_74, 2026-07-24 (NEW resist points that fill
out the 520 -> 565 V range not yet scanned). Same trigger/DREAM/HV setup as run_72/73,
all four dets equal. run_72 (530/545/560) + run_73 (538/553) already cover 530-560 V
densely; run_74 EXTENDS the ends (down to 520, up to 565) and interleaves the interior
so run_72+73+74 give a fine ~4 V ladder 520/524/528/530/534/538/542/545/549/553/557/
560/565 V at drift 600 V.
BEAM-OFF cosmic scint-Singles, drift 600 V, 15 min sub-runs.

Ungated cosmics trigger of run_68 (run_config_cosmics_resist_scan.py); resist points
are the 8 distinct values not covered by run_72/73, ordered spread-first so a 1-hour
access already spans the whole range and the 2nd hour refines it.
...
HV GRID — drift 600 V fixed on all four dets (card 9 ch0-3). Resist (card 5
  ch1-4) scans 8 NEW points, ALL FOUR detectors at the same value (operator
  2026-07-24 explicitly chose D equal, no offset — set DET_D_OFFSET=10 to
  reinstate the usual det D -10 V margin).

  ⚠ det D: the top point (565 V) is above the 2026-07-17 beam-off stress ceiling
  recorded for D (475 V), though D held fine at 560 V in run_72 (559.5 V, 3.98 µA,
  no trip) and run_68 ran A/B/C to 570 V. 565 is only 5 V over run_72's proven
  point; still, D pins the ceiling and tripped its drift on 07-23. The 565 V point
  is 2nd in the order — watch the HV alerts early; if D sparks, re-run with
  DET_D_OFFSET=10.

ORDERING — single pass of 8 distinct points, spread-first (N_CYCLES=1):
    520 -> 565 -> 542 -> 528  (first ~1 h: bottom, top, middle, low-mid)
    -> 549 -> 534 -> 557 -> 524  (2nd ~1 h: interleave the ladder)
  8 sub-runs x 15 min = 2 h total; the first 4 (~1 h) already span 520-565 so a
  1-hour access is complete-and-coarse and the 2nd hour refines it. Stop-anywhere;
  every sub-run is self-contained.
...
PRE-RUN (beam OFF, live run stopped, boards free — check config/n1081b_access/):
...
Launch:
  ./start_run.sh run_config_cosmics_hv_bounce_74.json
...
N_CYCLES = int(os.environ.get('N_CYCLES', '1'))     # single pass of 8 points x 15 min = 2 h

# ---- HV axes ----
DRIFT = 600                                # card 9 ch0-3, all dets, fixed
# 8 NEW resist points (not in run_72 {530,545,560} or run_73 {538,553}), spread-first:
# first 4 span 520-565 in ~1 h, last 4 interleave the ladder in the 2nd h.
RESIST = [520, 565, 542, 528, 549, 534, 557, 524]   # card 5 ch1-4
...
if __name__ == '__main__':
    c = Config()
    out = 'config/json_run_configs/run_config_cosmics_hv_bounce_74.json'
    c.write_to_file(out)
    dd = c.dream_daq_info
