...
# -*- coding: utf-8 -*-
"""
run_config_48h_hv_stats.py — run_76, 2026-07-25.
48-HOUR statistics run — IDENTICAL to run_75 (run_config_overnight_hv_stats.py) except
(1) 30 min sub-runs (was 15) and (2) ~48 h of cycles (N_CYCLES=10 -> 50 h; was 6 -> 15 h).
Modelled on run_71 with the mesh axis DROPPED: mesh charge-injection is OFF for the
entire run, and the resist ladder is a common 520 -> 560 V on all four detectors.
...
WHAT'S DIFFERENT vs run_71
  ...
  ⚠ det D: 550/560 V resist is well above every charted D ceiling (D "pins the ceiling",
     ...
     already 50 V lower than A/B/C. Over a 48 h unattended run the HV trip/deviation
    ...
    for cycle:                       # whole grid repeats until manually killed
      for drift in [700, 600]:       # fewest HV transitions
        for ri in [0..4]:            # resist ladder index 520->560
          acmeshOff (30 min)
  10 sub-runs/cycle x 30 min = 5.0 h/cycle. N_CYCLES=10 -> 100 sub-runs = 50 h of grid
  (>=48 h as requested, with headroom for beam gaps). Stop-anywhere / operator-killed;
  every sub-run is self-contained, so killing it at any point leaves a balanced grid.

DISK — ⚠ RAW at IPD 5 over ~48 h is the DOMINANT risk of this run. At run_75's measured
  beam-singles rate (~17.5 Hz, ~311 kB/event RAW n32 => ~5.4 MB/s) a 30 min sub-run is
  ~10 GB and 50 h of continuous beam is ~1 TB — near the 1.4 TB HDD on its own, and that
  is BEFORE the EOS backup copy. A 48 h RAW run therefore ABSOLUTELY REQUIRES the
  SSD->HDD->EOS pipeline (backup_watcher + space_manager) healthy and keeping up the WHOLE
  time; if EOS backup stalls, space_manager cannot free the HDD and the run will wedge on
  a full disk. CONFIRM both watchers are running and EOS is reachable before launch, and
  check disk headroom periodically. If disk gets tight, kill early (stop-anywhere) or run
  ZS via DREAM_ZS=1 (far smaller; the sensible choice if a 48 h RAW footprint won't fit).

FEU GUARDS — run_75 silently lost 51/60 sub-runs to a dead FEU (RunCtrl PedMemInit abort,
  but sub-runs still marked complete). The 07-24 fix is now standing: feu_health.py
  preflight + subrun_health.py byte check + daq_control fail-closed + 2 Telegram rules
  (memory feu-crate-guards-and-alarms). Essential for a 48 h unattended run — verify the
  guards are active before launch.
...
# ---- dwell + repeats ----
SUBRUN_MIN = float(os.environ.get('SUBRUN_MIN', '30'))     # 30 min per sub-run (operator, run_76)
N_CYCLES = int(os.environ.get('N_CYCLES', '10'))           # 10 sr x 30 min = 5 h/cycle -> 50 h (>=48 h)
...
class Config(BeamConfig):
    def _set_defaults(self, config_path=None):
        ...
        self.resume = False   # run_76 is a FRESH run (run_75's file was resume=True post-recovery)
        ...
        self.trigger = (
            f'48-HOUR statistics run, PS + SINGLES trigger, per-HV scan with MESH OFF '
            ...
            f'significant mesh effect). 30 min sub-runs, grid repeats {N_CYCLES}x (~48 h) '
...
if __name__ == '__main__':
    c = Config()
    out = 'config/json_run_configs/run_config_48h_hv_stats.json'
    c.write_to_file(out)

    n = len(c.sub_runs)
    data_min = sum(sr['run_time'] for sr in c.sub_runs)
    per_cycle = len(DRIFT_LADDER) * N_RESIST
    print(f'=== {c.run_name} — 48 h PS+singles resist x drift scan, MESH OFF ===')
    print(f'wrote       : {out}')
    ...
    print('Launch: ./start_run.sh run_config_48h_hv_stats.json')
