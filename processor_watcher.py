    ...
    if do_decode:
        ...
        with ThreadPoolExecutor(max_workers=n_threads) as pool:
            ...
            for fdf in all_fdf_paths:
                ...
                if root_path.exists():
                    continue
                futs[pool.submit(_decode_file, str(fdf), str(root_path),
                                 decode_exe)] = (fdf, root_path)
...
def _decode_file(fdf_path: str, root_path: str, decode_exe: str):
    """Decode one FDF. Returns the decoder's event count, or None on failure.

    The count comes from the decoder's own ``Events analysed : N`` line and is
    used by the caller to spot TRUNCATED output: every FEU of a file-group sees
    the same triggers, so their counts must agree. A decode process that died
    part-way used to leave a short ROOT which Step 1 then skipped forever
    (run_61 lost 68609 events across 84 files this way, run_62 10232).
    """
    print(f"[decode]  {os.path.basename(fdf_path)}")
    r = subprocess.run([decode_exe, fdf_path, root_path],
    ...
    if r.returncode != 0:
        print(f"[decode]  FAILED rc={r.returncode}: {os.path.basename(fdf_path)}")
    ...
    if not os.path.exists(root_path) or os.path.getsize(root_path) == 0:
        ...
        if os.path.exists(root_path):
            os.remove(root_path)
        return None
    m = re.search(r'Events analysed\s*:\s*(\d+)', r.stdout or '')
