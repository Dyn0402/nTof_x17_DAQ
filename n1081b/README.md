...
| 192.168.10.244 | 5 | 32429 | 2025.3.27.0 | scalers: counter ×4 — ⚠ **section C is held in Time-Tag 24/7 since 2026-07-29** by `tt_stream_supervisor.py` (trigger-timestamp stream). **Do not connect to .244**; see `TIMETAG_WATCHER.md` §The standing configuration |
...
| `tt_stream_supervisor.py` | **The standing Module-5 logger (running 24/7 since 2026-07-29).** Chains 6 h `tt_stream_qualify.py --section C` segments: one session and three stream commands per segment, zero reconnects, silent-start detection + 50 min-rest/probe recovery, Telegram, gzip + prune, disk guard. Owns .244 exclusively; run it in the tmux session `n1081b_timetag_watcher` so `poll_modules` skips the board. Stop with `touch config/tt_stream_supervisor.stop` — **never SIGKILL / kill-session mid-stream**. | server (board net) |
| `tt_stream_qualify.py` | The single-segment harness the supervisor drives (also usable standalone for a bounded capture). Pre/post counter baselines for any set of sections, one continuous stream, per-gap diagnosis, streamed-vs-expected report, verified restore. | server (board net) |
| `mod5_timetag_logger.py` | Superseded by the two above for production use. Streams per-edge timestamps from Module 5 (.244) to CSV via the Time Tag function — one row per input edge (host time, section, panel channel, board ns). `--section A..D` for one section, `--section cycle --dwell N` to rotate through all four (only one section can stream at a time — see Time-tag facts below). Restores the counter config on exit. | server (board net) |
