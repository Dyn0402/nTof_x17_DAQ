...
def next_run_num():
    """Claim the next run number. Thin wrapper — the allocator lives in run_num.py, which
    is also what the GUI Start Run path uses, so there is exactly one notion of 'next'."""
    return run_num.allocate()
